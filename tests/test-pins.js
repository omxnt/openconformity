...

{
  ...
  ok(editorSource.includes(".some((group) => group.attributes.some((held) => !isOutcome(held) && !isRationale(held) && (values[held.key] ?? '').trim() !== ''));") && editorSource.includes("relatedIds(model, subject, definition.relationship)).map((id) => ({ id, label: entityLabel(nodeOf(model, id)), state: 'linked' }))"), 'until the rating that writes a record holds a value, the field shows what is related now, unmarked, and only a written record is compared');
