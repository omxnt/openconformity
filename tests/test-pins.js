...

{
  ...
  ok(readFile('../app/modules/navigator.js').includes("(again ?? container.querySelector('.tree-row.selected'))?.focus({ preventScroll: true });") && readFile('../app/modules/navigator.js').includes("if (event.shiftKey && anchor !== null) store.pickAll(pickableBetween(anchor, row.id));") && readFile('../app/modules/store.js').includes("pickAll(ids) {"), 'a render keeps the tree where it was scrolled and the focus where it stood, and Shift and click picks the range from the last pick as one change');
  ok(readFile('../app/modules/library.js').includes("attributes: { type: 'button', role: 'switch', 'aria-checked': String(headings), id: 'library-headings' },") && readFile('../app/modules/library.js').includes("if (sectionsFresh) {\n      openSections = new Set([sections[0].name]);") && readFile('../app/style.css').includes('.toggle[aria-checked="true"] .toggle-knob { transform: translateX(16px); }'), "the picker's head carries Carbon's toggle for the headings a pick brings, and the preview opens its first section afresh for each highlight");
  ok(readFile('../app/modules/library.js').includes("togglePick(library, picks, node.id, alone, lone);") && readFile('../app/modules/library.js').includes("alone = event.altKey;") && readFile('../app/modules/library.js').includes("text: 'Alt and click picks one entity alone.'"), 'Alt and click on a box picks that one entity alone, and the picker says so');
  ok(!readFile('../app/modules/library.js').includes('presence(') && !readFile('../app/modules/library.js').includes('joinKey(') && readFile('../app/modules/library.js').includes("const result = relate(project, relationship.type, mapping.get(relationship.source), mapping.get(relationship.target));") && readFile('../app/modules/library.js').includes("return filedBeneath(library, null).filter((node) => node.kind === 'entity' && (picks.has(node.id) || carried.has(node.id)));"), 'an import is one way, nothing recognised as already there, the picks travelling with the relationships among them and never a folder');
