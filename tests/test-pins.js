...

{
  ...
  ok(!readFile('../app/modules/editor.js').includes('cell-note') && readFile('../app/modules/editor.js').includes("cell.appendChild(el('p', { className: 'field-warning' }, [statusIcon('medium'), el('span', { text: changedText(definition, current?.type ?? '') })]));") && readFile('../app/modules/editor.js').includes("const cellElement = el('div', { className: 'cell' });\n    questioned(group.name, cellElement);") && readFile('../app/style.css').includes('.field-warning {'), "a changed record warns under the field it puts in question, the rating or the choice, as Carbon's warning helper, and says nothing under the record itself");
