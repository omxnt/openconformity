...

{
  ...
  ok(readFile('../app/modules/library.js').includes("for (const [value, label] of [['headings', 'With headings'], ['beneath', 'Beneath only'], ['alone', 'Alone']]) {") && readFile('../app/modules/library.js').includes("if (sectionsFresh) {\n      openSections = new Set([sections[0].name]);") && readFile('../app/style.css').includes('.pick-mode-item[aria-pressed="true"]'), "the picker's head carries Carbon's content switcher saying what a pick brings, and the preview opens its first section afresh for each highlight");
  ok(readFile('../app/modules/library.js').includes("togglePick(library, picks, node.id, alone ? 'alone' : mode, bare);") && readFile('../app/modules/library.js').includes("alone = event.altKey;") && readFile('../app/modules/library.js').includes("text: 'Alt and click picks one entity alone.'"), 'Alt and click on a box picks that one entity alone, and the picker says so');
