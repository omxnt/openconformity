...

{
  ...
  ok(readFile('../app/modules/app.js').includes("if (editor.editing()) editor.submit();\n    else actions.find((action) => action.id === 'save')?.run({});") && readFile('../app/modules/app.js').includes("if (picker !== null && picker.picks.length > 0) flows.completeRelate(picker.subject, picker.picks);") && readFile('../app/modules/app.js').includes("document.querySelector('#drawing-head .button-primary')?.click();") && readFile('../app/modules/app.js').includes("const filter = document.getElementById('navigator-filter');") && readFile('../app/modules/app.js').includes("} else if (key === 'y' && event.ctrlKey && !event.metaKey) {") && readFile('../app/modules/navigator.js').includes("} else if (event.key === 'F2') {") && readFile('../app/modules/navigator.js').includes("} else if (event.key === 'Backspace' && event.metaKey) {") && readFile('../app/modules/actions.js').includes("const REDO_HINT = APPLE ? '⇧⌘Z' : 'Ctrl Y';"), 'the keys the platforms agree on: S saves the edit or the file, Enter finishes an edit, a picking or a diagram, F reaches the filter, Y redoes on Windows, F2 edits or renames, ⌘⌫ deletes on a Mac, and the Edit menu shows every hint as the platform writes it');
  ok(readFile('../app/modules/graph.js').includes("group.setAttribute('aria-label', `Unpick ${unpickId}`);") && readFile('../app/modules/graph.js').includes("if (event.key !== 'Enter' && event.key !== ' ') return;\n        event.preventDefault();\n        store.togglePick(unpickId);"), 'a pending edge in the graph is a button the keyboard reaches, so letting a pick go needs no pointer');
  ok(readFile('../app/modules/drawing-editor.js').includes("text: `Diagrams are created and edited in draw.io, a third-party service embedded in openconformity. By continuing, you load the editor from ${EDITOR_ORIGIN.replace('https://', '')}. The diagram you edit will be sent to draw.io, since draw.io provides the editor.`") && readFile('../app/style.css').includes('.consent p { margin: 0 0 12px; text-wrap: pretty; }'), 'the consent says what draw.io is, what continuing loads and from where, and what will be sent to it and why, in one paragraph');
...

{
  ...
  ok(editor.includes("body: el('div', { className: 'drawing-large' }, [el('img', { attributes: { src: dataUrl(text), alt: `Diagram of ${subject()}` } })]),\n        actions: [],") && readFile('../app/modules/dialog.js').includes("...(buttons.length > 0 ? [el('div', { className: 'dialog-actions' }, buttons)] : []),"), 'at full size it is a passive dialog, no footer, closed by its X, Escape or a click outside');
  {
    ...
    ok(editorModule.includes("export function createDrawingSurface({ workspace, pane, head, body, store }) {") && editorModule.includes("workspace.classList.add('drawing');") && editorModule.includes("store.setDrawingOpen(true);") && !editorModule.includes("title: `Diagram of ${subject}`,\n    body: host,") && readFile('../app/style.css').includes('.workspace.drawing > :not(.pane-drawing) { display: none; }') && readFile('../app/style.css').includes('.pane-drawing[hidden] { display: none; }') && readFile('../app/modules/actions.js').includes("enabled: () => !store.drawingOpen() && action.enabled()") && readFile('../app/modules/app.js').includes("if (store.drawingOpen()) {\n    if (key !== 'enter') return;"), 'the editor stands over the whole workspace on a pane of its own, never in a dialog, the panes hidden beneath it and every action and shortcut held still until Apply or Cancel');
  ...
  ok(readFile('../app/style.css').includes('.tag-trigger:focus-visible .tooltip { display: block; animation: tooltip-in 70ms var(--motion-standard) 100ms both; }') && readFile('../app/style.css').includes('.tag.reasoned { text-decoration: underline dotted; text-underline-offset: 3px; }'), "shown on hovering or focusing it, as the help glyph's is");
