...

{
  ...
  ok(shell.includes("const fileGroups = ['project', 'import', 'example', 'browser']"), 'the File menu parts the import, the example and the clearing of browser data behind separators');
...

{
  ...
  ok(readFile('../app/modules/shell.js').includes("const fileGroups = ['project', 'import', 'example', 'browser'];"), 'the File menu holds the import into the open project between the file actions and the example, since opening replaces your work and importing adds to it');
  ok(readFile('../app/modules/flows.js').includes("const chosen = await openLibraryPicker({ dialogs, store, libraries: LIBRARIES });") && readFile('../app/modules/flows.js').includes("const outcome = store.commit((model) => importInto(model, chosen.library, chosen.picks, chosen.target));"), 'an import is the picker over the workspace, then one committed change the history can undo');
  ok(readFile('../app/modules/library.js').includes("const present = presence(project, library);") && readFile('../app/modules/library.js').includes("const parent = above && above.kind === 'entity' ? (mapping.get(above.id) ?? targetId) : targetId;"), "a copy recognises by reference what the project holds and lands a clause under the project's own act");
  ok(readFile('../app/library/index.js').includes("export const LIBRARIES = [") && !readFile('../app/library/index.js').includes('fetch(') && !readFile('../app/modules/library.js').includes('fetch('), 'the built-in libraries load with the software and are fetched from nowhere');
...

{
  ...
  ok(editor.includes("body: el('div', { className: 'drawing-large' }, [el('img', { attributes: { src: dataUrl(text), alt: `Diagram of ${subject()}` } })]),\n        actions: [],") && readFile('../app/modules/dialog.js').includes("...(buttons.length > 0 ? [el('div', { className: 'dialog-actions' }, buttons)] : []),"), 'at full size it is a passive dialog, no footer, closed by its X, Escape or a click outside');
  {
    ...
    ok(editorModule.includes("window.addEventListener('message', handler);") && editorModule.includes("window.removeEventListener('message', handler);") && editorModule.includes("const message = acceptMessage(event, frame.contentWindow);"), 'the listener lives only while the dialog is open, and hears only what the filter admits');
...

{
  ...
  ok(relationshipsSource.includes("if (store.hasProject() && store.messagesOpen()) {\n      const found = findings(store.model());\n      renderMessagesHead(found.length);") && relationshipsSource.includes("messagesHost.appendChild(emptyState('Nothing to revisit', 'Every record matches what is related.'));"), 'the messages stand over the pane whatever is selected, with a head of their own, and say so when there are none');
