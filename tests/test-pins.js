...

{
  ...
  ok(readFile('../app/library/index.js').includes("export const LIBRARIES = APP_LIBRARY.folders") && readFile('../app/library/index.js').includes("project: catalogueOf(APP_LIBRARY, folder.id)") && readFile('../app/library/app-library.js').includes("export const APP_LIBRARY = {") && !readFile('../app/library/index.js').includes('fetch(') && !readFile('../app/modules/library.js').includes('fetch('), 'the built-in libraries load with the software and are fetched from nowhere');
