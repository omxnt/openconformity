...

{
  ...
  ok(readFile('../app/modules/drawing-editor.js').includes("text: `Diagrams are created and edited in draw.io, a third-party service embedded in this software. The editor is loaded from ${EDITOR_ORIGIN.replace('https://', '')}. The diagram being edited is sent to draw.io.`") && readFile('../app/style.css').includes('.consent p { margin: 0 0 12px; text-wrap: pretty; }'), 'the consent names the service, its origin and the one thing sent to it, in one paragraph, promising nothing about the rest');
