...

{
  ...
  ok(readFile('../app/modules/relationships.js').includes("attributes: { type: 'button', role: 'checkbox', 'aria-checked': state === 'checked' ? 'true' : state === 'mixed' ? 'mixed' : 'false', 'aria-label': 'Pick all shown' },") && readFile('../app/modules/relationships.js').includes("if (state === 'checked') store.unpickAll(shown);\n        else store.pickAll(shown);") && readFile('../app/modules/library.js').includes("const all = el('button', { className: 'head-all', attributes: { type: 'button', role: 'checkbox', 'aria-checked': 'false', 'aria-label': 'Pick all shown' } }, [el('span', { className: 'checkbox' }, [icon('i-checkmark')]), count]);") && readFile('../app/style.css').includes('.head-all {'), "each picker's head carries the select-all box with the count beside it, checked, mixed or empty for what is shown, picking all shown or clearing them");
