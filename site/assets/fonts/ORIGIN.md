...
the heading styles. The page reaches no further up the scale than `$heading-05`,
which is Regular, so Light is not vendored. Medium 500 is not a weight the scale
uses, and is not vendored either.
