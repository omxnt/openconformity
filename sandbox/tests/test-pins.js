...

{
  ...
  const editor = readFile('../app/editor.js');
  ok(editor.includes("if (control.closest('.cell-group[hidden]')) continue;"), 'a control under a group hidden by its condition is not saved');
...

{
  ...
  ok(!editor.includes('data-related') && !editor.includes('relatedDiff') && editor.includes("const live = current ? relatedIds(store.model(), current.id, definition.relationship) : [];"), 'nothing of the list is stored: it is read from the model, never recorded');
  ok(!page.includes('id="i-rate"') && !origin.includes('i-rate'), 'the rating field has no glyph of its own: it trails the edit pencil');
  ok(editor.includes("live.length === 0\n        ? el('div', { className: 'cell-value empty', text: 'None linked.' })"), 'an empty list says so, as an empty value');
  ...
  ok(editor.includes("className: 'entity-row', attributes: { type: 'button' } }, [") && editor.includes("icon(TYPE_ICONS[entity.type], ENTITY_TYPES[entity.type].pillar),"), 'shown as the tree shows an entity: its glyph in the pillar colour, its identifier, its label');
