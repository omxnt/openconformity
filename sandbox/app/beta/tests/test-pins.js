...

{
  ...
  ok(!editor.includes('download(') && !editor.includes("type: 'file'") && editor.includes("ghost(text === '' ? 'Create in draw.io' : 'Edit in draw.io', text === '' ? 'i-new-entity' : 'i-edit', async () => {") && editor.includes("if (text !== '') actions.push(ghost('Remove', 'i-delete', () => hold(''), true));") && editor.includes("body.appendChild(el('div', { className: 'drawing-refused', text: `This drawing cannot be shown. It ${verdict.reason}.` }));"), 'a drawing is created, edited and removed in an edit and enters only from the editor, never from a file; one that fails the check says why and stays');
  ok(editor.includes("actions: [{ label: 'Close', value: null, kind: 'primary', default: true }],"), 'at full size it closes, and nothing else');
  {
    const modules = readFile('../app/index.html').match(/src="modules\/app\.js"/) ? ['about', 'actions', 'app', 'attributes', 'dialog', 'dom', 'drawing', 'drawing-editor', 'editor', 'example', 'files', 'flows', 'graph', 'history', 'icons', 'menu', 'metamodel', 'model', 'multiselect', 'navigator', 'overlay', 'project', 'queries', 'rating', 'relate', 'relationships', 'risk', 'shell', 'store', 'validator', 'view-risk', 'views'] : [];
    ...
    ok(editorModule.includes("post({ action: 'load', xml: embeddedModel(drawing) ?? '', autosave: 1, dark: dark === true });") && !editorModule.includes('store.model()') && !editorModule.includes('store.project()'), "the one drawing's model is what is handed over, nothing of the project");
