...
and at 32 px otherwise; no glyph drawn here at 32 px ships a 16 px cut. The SVG
files are the repository sources, in which a shape may be a `rect` or a
`polygon` that the package build converts to an equivalent `path`; the sprite
carries the built form.

The entity icons are chosen for their silhouettes: eighteen types have eighteen
distinct outer shapes, so a row in a dense tree is told apart at 16 px without
reading the marks inside a shared frame. The two pairs the metamodel splits are
told apart the same way rather than by a mark added to a shared shape: a
harmonised standard is an open book and another standard a bound volume seen
from its spine; a harmonised requirement is a ribbon and another requirement a
label.
